...
function Square(props) {
    ...
    function hover() {
        return(
            store.getState().userShoot&&context.whoIsThisField==="AI"? "hover ":null
        )}
    return (
        <div
            className = {"square " + userShips() + shadowShips() + cantShips() + failShot() + crackShip() + hover()}
            onClick = {props.waitForClick? ((context.whoIsThisField==="AI")?((store.getState().userShoot)?props.battleOnClick:null) : props.onClick) : null}
